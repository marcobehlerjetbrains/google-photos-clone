...
public class MediaTest {
    ...
    MediaScanner mediaScanner = new MediaScanner(null, null, null);
        ...
        try (ScanResult scanResult = new ClassGraph().scan()) {
            ...
            return images.stream().map(image -> DynamicTest.dynamicTest(image, () -> {
                ...
                try (InputStream json = MediaTest.class.getResourceAsStream("/" + filename + ".json");) {
                    ...
                    // 1. dimensions
                    MediaScanner.Dimensions actualDimensions = mediaScanner.getDimensions(actualMetadata);
                    ...
                    // 2. creation time
                    LocalDateTime creationTime = mediaScanner.getCreationTime(file, actualMetadata);
                    assertThat(creationTime).as("creation time").isEqualToIgnoringSeconds(expectedMetadata.date());

                    // 3. location
                    Location actualLocation = mediaScanner.getLocation(actualMetadata);
