...
public class NewMediaListener {
    ...
    public NewMediaListener(ImageMagick imageMagick, EntityManagerFactory emf) {
        this.imageMagick = imageMagick;
        this.emf = emf;
    }

    @JmsListener(destination = "media")
    public void receiveMessage(Map<String, String> hashedImage) throws URISyntaxException {
        String uri = hashedImage.get("uri");
        String hash = hashedImage.get("hash");


        Path image = Path.of(new URI(uri));
        ...
        try (InputStream is = Files.newInputStream(image)) {
            ...
            emf.unwrap(SessionFactory.class).inStatelessSession(ss -> {
                Media media = null;
                media = new Media(hash, image.getFileName().toString(), creationTime, image.toUri().toString(), location);
                ss.insert(media);
            });
